...
export const showNotification = ({ title, type, message }: ShowNotificationProps): void => {
...
export const errorNotification = (err: any): void => {
  if (err.response) {
    const data: ErrorType = err.response.data;

    showNotification({ title: data.error || 'Error', type: 'danger', message: data.exception || 'Error' });
  } else {
    throw err;
  }
};
